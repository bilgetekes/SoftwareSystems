...
public class Hotel {
	...
	private PricedRoom secondroomofhotel;
	...
	public Hotel(String x) {
		...
		secondroomofhotel = new PricedRoom(102, 20.0, 5.0 );
		name = x ;
	...
	public double getBill(String nameofguest, int numberofnightspend, Printer printer) {
		Bill bill = new Bill (printer);
		Guest x = guestnametoguest(nameofguest);
		double result = bill.sumofbill;
		
		if(x.getRoom() instanceof PricedRoom) {
			Bill.Item a =(Item) x.getRoom();
			if( x.getRoom().getSafe() != null && x.getRoom().getSafe() instanceof PricedSafe) {
				for (int i=0; i<= numberofnightspend;i++) {
				Bill.Item b =(Item) x.getRoom().getSafe();
				bill.addItem(b);
				bill.addItem(a);
				}
			}
